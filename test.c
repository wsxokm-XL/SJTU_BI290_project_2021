...
char ss[MAXN]; int lenof;//主序列（如基因组序列和长度
char s0[amount][MAX];int lenofs[amount]; //比对的序列（可以只用一个字符串简化）和长度
...
scanf("%s",ss);
lenof=strlen(ss);
strncat(s+1,ss,lenof);//输入主序列并将主序列储存在ss中
...
SuffixSort(N,s);//求后缀树组
GetHeight(N,rank,sa,s);//求height数组

printf("\nResult:\n\n");
int link=lenof+1,target; //link：@结束的下标，target：匹配的起始下标
for(i=0;i<amount;i++)
{
    printf("s%d:\n",i+1);
    target=sa[rank[link+1]+1];//printf("link=%d,target=%d,lenofs=%d,Height[rank[target]]=%d\n",link,target,lenofs[i],Height[rank[target]]);
    int j=0,jj=0;//j：判断，jj：判断是否found    //可以简化
    while(j==0)
    {
